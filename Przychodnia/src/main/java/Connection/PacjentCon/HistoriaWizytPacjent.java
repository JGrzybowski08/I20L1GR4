...
public class HistoriaWizytPacjent {
    /**
    ...
    public static ObservableList<ModelAktualneWizytyPacjent> FiltrowaneWizytyGet(String FiltrCB, String FiltrTF, int PacjentID) {
        ...
        String sql = "SELECT wizyty.Data, wizyty.Godzina, wizyty.Opis, wizyty.Status, pacjenci.Imie, pacjenci.Nazwisko, lekarze.Imie, lekarze.Nazwisko FROM wizyty, pacjenci, lekarze WHERE wizyty.Pacjent_ID = pacjenci.Pacjent_ID AND wizyty.Pacjent_ID = "+ PacjentID +" AND "+ FiltrCB +" = '"+ FiltrTF +"' AND wizyty.Lekarz_ID = lekarze.Lekarz_ID AND wizyty.Data < '"+ dtf.format(now) +"' ORDER BY wizyty.Data DESC, wizyty.Godzina DESC";


        switch(FiltrCB){
            case "Lekarz":
                IiN = FiltrTF.split(" ");
                sql = "SELECT wizyty.Data, wizyty.Godzina, wizyty.Opis, wizyty.Status, pacjenci.Imie, pacjenci.Nazwisko, lekarze.Imie, lekarze.Nazwisko FROM wizyty, pacjenci, lekarze WHERE wizyty.Pacjent_ID = pacjenci.Pacjent_ID AND wizyty.Pacjent_ID = "+ PacjentID +" AND lekarze.Imie = '"+ IiN[0] + "' AND lekarze.Nazwisko = '"+ IiN[1] +"' AND wizyty.Lekarz_ID = lekarze.Lekarz_ID AND wizyty.Data < '"+ dtf.format(now) +"' ORDER BY wizyty.Data DESC, wizyty.Godzina DESC";
                break;
        }
